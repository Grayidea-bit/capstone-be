    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if previous_diff_for_prompt and previous_commit_number is not None:
                combined_diff_for_prompt += f"**上下文：前一個 Commit (序號: {previous_commit_number}, SHA: {previous_commit_sha}) 的 Diff 摘要:**\n```diff\n{previous_diff_for_prompt}\n```\n\n"
            elif previous_diff_for_prompt:
                combined_diff_for_prompt += f"**上下文：前一個 Commit (SHA: {previous_commit_sha}) 的 Diff 摘要:**\n```diff\n{previous_diff_for_prompt}\n```\n\n"
            else:
                combined_diff_for_prompt += "沒有找到前一個 commit 的 diff，或者這是倉庫中的第一個有效 commit。\n\n"
...
"""
            ...
            logger.info(
                f"送往 AI 服務的分析提示詞 (模型: sonar-deep-research, 提示詞長度約: {len(prompt)} 字元): {log_prompt}"
            )
            ...
            logger.info(
                f"AI 服務分析結果 (模型: sonar-deep-research): '{log_analysis}'"
            )
            return {
                "sha": sha,
